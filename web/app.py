...
3. **本规则为无限流、重战斗体系**：宣读技能、贴怪物数值、报检定 DC 都是 KP 的正常工作。判断叙事力时把"机制宣读"与"描写/旁白"分开看，重点评估描写部分的质量，而不是因为机制密集就扣分。
4. 评分必须基于样本中的**具体证据**，不要凭借数据数字脑补。
...
5. **本规则为无限流、重战斗体系**：贴技能描述、读规则数值、战斗轮抛骰子都是正常游戏行为，不直接判低分。判断扮演分时把"机制类发言"和"扮演类发言"分开看，重点评估扮演类发言的质量和占比，而不是因为出现机制内容就扣分。
6. 评分必须基于样本中的**具体证据**，不要凭借数据数字脑补；若整局几乎没有任何扮演性表达（只有机制操作、无角色台词/动作/心理），才能直接判 30-50。
...
- **55-64 平淡**: 多数 IC 发言为信息陈述或动作交代，少量角色色彩。
- **45-54 一般**: 扮演类发言稀少且模板化；主要靠机制操作存在感。
- **35-44 浮于表面**: 角色名仅是占位符；几乎没有性格或世界观体现。
- **25-34 出戏严重**: 在 IC 场景中也很难看出在扮演谁。
- **15-24 几乎无扮演**: 全程只发规则讨论、表情包、调侃，无任何角色性内容。
- **0-14 完全失格**: 无任何扮演意图。
...
3. 同时指出 1 个亮点 + 1 个不足
4. 简体中文，80-160 字
5. 注意：本规则为无限流重战斗体系，技能/数值/检定属于正常机制内容，不要因此判低分"""
